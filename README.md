...
東京大学物性研究所(以下、物性研)に限らず、スパコンを保有する組織は、目的に合わせて複数のスパコンを持っているのが普通である。物性研は、2021年現在「システムB」と「システムC」と呼ばれる。以下、物性研スパコンのシステムBの使い方を外観する。
...
アカウントは「kXXXXYY」という形になっている。XXXXがプロジェクト番号、YYがプロジェクト中のメンバー番号で、プロジェクトリーダーが「00」、以下登録順に「01」「02」と続く。物性研スパコンのサーバ名は、システムBが「ohtaka」、システムCが「enaga」である。ドメインは「issp.u-tokyo.ac.jp」となる。例えばプロジェクト`k0123`の、アカウント番号`01`番の人がログインするためには

```sh
ssh k012301@ohtaka.issp.u-tokyo.ac.jp -AY
```
